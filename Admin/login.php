...
$errorMessage = ""; // Initialize an empty error message
    ...
    if($result->num_rows==1){
        $_SESSION['username'] = $username;      //new code;
        header("Location: index.php");      //admin"s index page;
        exit();
    }
    else {
        
        $errorMessage = '<script>alert("Invalid username or password. Please try again.");</script>';
    }
...
<div class="inputContainer">
    ...
    <input placeholder="Password" id="password" class="inputField" type="password" name="password" required>
</div>
